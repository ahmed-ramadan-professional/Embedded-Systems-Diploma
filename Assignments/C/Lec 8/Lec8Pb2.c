...
int arraySearchForLastOccurrence(int haystack[], int size, int needle) {
    int index = -1;
...
void arrayPrintWithHighlight(int arr[], int size, int marker) {
    ...
    printf("Value [%d] was last found at index <%d> or position <%d>\n", arr[marker], marker, marker + 1);
}
...
int main(int argc, char const *argv[]) {
    ...
    while (1) {
        ...
        printf("This program will try to find the last occurrence index of a value you entre in an array you will provide.\n");
        ...
        result = arraySearchForLastOccurrence(haystack, size, needle);
